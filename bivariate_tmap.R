...
bivariate_choropleth <- function (
  ...
  bivmap <- get_bivariate_choropleth(
    ...
    bvColors=bvColors,
    bivmap_label_point=bivmap_label_point,
    bivmap_label_col = bivmap_label_col,
    footnote=footnote,
    title=title
  )
  ...
  vp <- viewport(x=.17, y=.78, width=.3, height=.3, angle = 0) # change angle for diamond but labelling becomes a bit weird...
  pushViewport(vp)
  ...
  print(levelplot(
    ...
    xlab=list(label=bivmap_labels[1],cex=1.5, fontface=3),
    ylab=list(label=bivmap_labels[2],cex=1.5,fontface=3),
    cuts=8,
    colorkey=FALSE,
    scales=list(draw=0)),
    newpage=FALSE)
...
get_bivariate_choropleth <- function (
  ...
  bivmap_dataset <- as(bivmap_dataset,"Spatial")
  ...
  if (!is.null(basemap)) {
    ...
    if (isTRUE(northarrow)){
      bivmap <- bivmap + tm_compass(position=north_pos, size=5, text.size=1.4)
    }
  } else {
    bivmap <-
      tm_shape(bivmap_sdf) +
    ...
    if (isTRUE(northarrow)){
      bivmap <- bivmap + tm_compass(position=north_pos, size=7, text.size=2)
    }
  }

  if (!is.null(bivmap_label_point)){
    bivmap <- bivmap +
      tm_shape(bivmap_label_point)+
      tm_dots('black', size=0.2, legend.show=F) +
      tm_text(bivmap_label_col, auto.placement=F, size=1.5, just=c('center'),
              fontface = 3, ymod=-1)
  }
  ...
  if (bivmap_scale) {
    bivmap <- bivmap  +
      # Add scale bar
      tm_scale_bar(
        position=scale_pos, width=0.25, text.size=1.4)
  }
