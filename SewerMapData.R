...
# ------ READ DATA --------------
process_SSO_Data <- function(overwrite=F){
  if (file.exists('Join_Sewer_Serv_Data.rds') & overwrite==F){
  ...
  # --- create
  if (file.exists('Sewer_Serv_Areas_cent.rds') & overwrite==F){
...
return(list(SSA_Data= Join_Sewer_Serv_Data,
            SSA_centroid = Sewer_Serv_Areas_cent))
}
